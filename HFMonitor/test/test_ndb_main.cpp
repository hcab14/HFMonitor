...
public:
  ...
  typedef frequency_vector<double> fv_type;
  // frequency -> deques with history
  typedef frequency_vector<std::deque<double> > fv_hist_type;
  ...
  void process_iq(processor::service_iq::sptr sp,
    ...
    if (ptime_history_.size() > 1) {
      std::cout << "history size: " << ptime_history_.back()-ptime_history_.front() << std::endl;
      while (ptime_history_.back()-ptime_history_.front() > max_time_interval_) {
        for (fv_hist_type::iterator i(fv_history_.begin()), end(fv_history_.end()); i!=end; ++i)
          i->second.pop_front();
    ...
    for (fv_hist_type::const_iterator i(fv_history_.begin()), iend(fv_history_.end()); i!=iend; ++i) {
      ...
      //       const std::string fn(str(boost::format("%.0f.dat") % f));
      const std::deque<double>& ft(i->second);
      size_t epoch_counter(0);
      for (std::deque<double>::const_iterator j(ft.begin()), jend(ft.end()); j!=jend; ++j, ++epoch_counter) {
