...
namespace FiFiSDR {
  ...
  public:
    ...
    public:
      ...
      presel_entry(double freq1_MHz, double freq2_MHz, boost::uint8_t pattern);
      double freq1_MHz() const; // [MHz]
      double freq2_MHz() const; // [MHz]
      double pattern() const { return pattern_; }

      friend bool operator==(const presel_entry& pe1, const presel_entry& pe2) {
        return (pe1.freq1_MHz() == pe2.freq1_MHz() &&
                pe1.freq2_MHz() == pe2.freq2_MHz() &&
                pe1.pattern()   == pe2.pattern());
      }          
      friend std::ostream& operator<<(std::ostream&, const presel_entry&);
    private:
      boost::uint32_t freq1_; // [counters]
      boost::uint32_t freq2_; // [counters]
