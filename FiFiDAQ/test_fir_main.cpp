...
int main()
{
  ...
  filter::fir::lowpass<float_type> fir(m);
  fir.design(0.05, 0.02);
#if 1
  const size_t l(1024*8);

  filter::fir::overlap_save<float_type> os(l,m);
  os.update_filter_coeff(fir.coeff());

  filter::fir::overlap_save<float_type>::complex_vector_type buffer(l);

  std::cout << "out = [\n";
  for (size_t i=0; i<2; ++i) {
    for (size_t j=0; j<l; ++j) 
      buffer[j]= filter::fir::overlap_save<float_type>::complex_type(drand48()-0.5, drand48()-0.5);

    const filter::fir::overlap_save<float_type>::complex_vector_type out(os.proc(buffer));
    for (size_t j=0; j<l; ++j) {
  ...
  std::cout << "];\n";
#endif
  typedef filter::fir::overlap_save<float_type> ols;
